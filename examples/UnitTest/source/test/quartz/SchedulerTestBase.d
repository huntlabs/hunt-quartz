...
class SchedulerTestBase {
    ...
    //     Scheduler sched = createScheduler("testAbilityToFireImmediatelyWhenStartedBefore", 2);
    //     JobDetail job1 = sched.getJobDetail(jobKey("job1"));
    // }
    ...
    void testAbilityToFireImmediatelyWhenStartedBefore(){
		...
		JobDetail job1 = JobBuilder.newJob(typeid(TestJobWithSync))
                .withIdentity("job1")
			    .usingJobData("jobDetail1", "job's data")
                // .storeDurably()
                .build();

		Trigger trigger1 = TriggerBuilder!(Trigger).newTrigger()
                .forJob(job1)
                .build(); 
...
class TestJobWithSync : Job {
    ...
    void execute(JobExecutionContext context){
        ...
        try {
            ...
            string jobData = dataMap.getString("jobDetail1");
            tracef("jobDetail1: %s", jobData);
            info("a job executed.");
            // barrier.await(TEST_TIMEOUT_SECONDS, TimeUnit.SECONDS);
        } catch (Throwable e) {
            error(e.msg);
