...
import hunt.quartz.Trigger.TriggerTimeComparator;
...
import static hunt.quartz.impl.matchers.EverythingMatcher.allTriggers;
...
class RAMJobStore implements JobStore {
    ...
    void storeJob(JobDetail newJob,
            bool replaceExisting) throws ObjectAlreadyExistsException {
    ...
    void storeTrigger(OperableTrigger newTrigger,
            bool replaceExisting) throws JobPersistenceException {
     ...
     */
    bool replaceTrigger(TriggerKey triggerKey, OperableTrigger newTrigger) throws JobPersistenceException {
    ...
    JobDetail retrieveJob(JobKey jobKey) {
        synchronized(lock) {
            ...
            return (jw !is null) ? (JobDetail)jw.jobDetail.clone() : null;
        }
    }
    ...
    OperableTrigger retrieveTrigger(TriggerKey triggerKey) {
        synchronized(lock) {
            ...
            return (tw !is null) ? (OperableTrigger)tw.getTrigger().clone() : null;
        }
    }
     ...
     * @throws JobPersistenceException
     */
    bool checkExists(JobKey jobKey) throws JobPersistenceException {
        synchronized(lock) {
            JobWrapper jw = jobsByKey.get(jobKey);
            return (jw !is null);
        }
     ...
     * @throws JobPersistenceException
     */
    bool checkExists(TriggerKey triggerKey) throws JobPersistenceException {
        synchronized(lock) {
            TriggerWrapper tw = triggersByKey.get(triggerKey);
    ...
    Set!(JobKey) getJobKeys(GroupMatcher!(JobKey) matcher) {
            ...
            switch(operator) {
                ...
                default:
                    for (Map.Entry!(string, HashMap!(JobKey, JobWrapper)) entry : jobsByGroup.entrySet()) {
                        if(operator.evaluate(entry.getKey(), compareToValue) && entry.getValue() !is null) {
                            if(outList is null) {
                                outList = new HashSet!(JobKey)();
                            }
        ...
        return outList is null ? java.container.Collections.<JobKey>emptySet() : outList;
    }
    ...
    Set!(TriggerKey) getTriggerKeys(GroupMatcher!(TriggerKey) matcher) {
        ...
        return outList is null ? Collections.<TriggerKey>emptySet() : outList;
    }
