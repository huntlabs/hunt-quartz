...
import hunt.quartz.impl.StdJobRunShellFactory;
...
class DirectSchedulerFactory : SchedulerFactory {
    ...
    void createVolatileScheduler(int maxThreads) {
        SimpleThreadPool threadPool = new SimpleThreadPool(maxThreads, Thread.PRIORITY_DEFAULT);
    ...
    void createScheduler(string schedulerName,
        ...
        if (schedulerPluginMap !is null) {
            foreach (string key, SchedulerPlugin value; schedulerPluginMap) {
                value.initialize(key, scheduler);
            }
        }
