...
module hunt.quartz.impl.jdbcjobstore.DB2v6Delegate;
 ...
 * @author James House
 */
class DB2v6Delegate : StdJDBCDelegate {
    @SuppressWarnings("hiding")
    enum string SELECT_NUM_JOBS = "SELECT COUNT(*) FROM "
            + TABLE_PREFIX_SUBST + TABLE_JOB_DETAILS
            ~ " WHERE " ~ COL_SCHEDULER_NAME ~ " = " ~ SCHED_NAME_SUBST;

    @SuppressWarnings("hiding")
    enum string SELECT_NUM_TRIGGERS_FOR_JOB = "SELECT COUNT(*) FROM "
            + TABLE_PREFIX_SUBST
            + TABLE_TRIGGERS
            ~ " WHERE "
    ...
    @SuppressWarnings("hiding")
    enum string SELECT_NUM_TRIGGERS = "SELECT COUNT(*) FROM "
            + TABLE_PREFIX_SUBST + TABLE_TRIGGERS
            ~ " WHERE " ~ COL_SCHEDULER_NAME ~ " = " ~ SCHED_NAME_SUBST;

    @SuppressWarnings("hiding")
    enum string SELECT_NUM_CALENDARS = "SELECT COUNT(*) FROM "
